          ...
          <div className="px-6 pt-6 max-w-2xl mx-auto w-full">
            ...
            <div className="h-1 rounded-full" style={{ background: style.cardColor }}>
              <div
                ...
                style={{
                  width: `${((page + 1) / questions.length) * 100}%`,
          ...
          <div className="max-w-2xl w-full">
            ...
            <div className="mt-8 flex items-center gap-3">
              <button
                ...
                disabled={page === 0}
                className="px-5 py-2.5 rounded-lg border text-sm disabled:opacity-30"
                style={{ borderColor: style.cardColor }}
              >
                ← Back
              </button>
              {!isLast ? (
                <button
                  ...
                  className="px-5 py-2.5 rounded-lg text-sm text-white inline-flex items-center gap-1.5"
                ...
                <button
                  onClick={submit}
                  className="px-5 py-2.5 rounded-lg text-sm text-white font-semibold"
                  style={{ background: style.primaryColor }}
                >
                  Submit ✓
                </button>
              ...
              <div className="ml-auto flex gap-1.5">
                {questions.map((_, i) => (
                  <span
                    ...
                    style={{ background: i === page ? style.primaryColor : style.cardColor }}
                  />
                ))}
              </div>
            </div>
    ...
    <div className="p-6 rounded-xl" style={{ background: style.cardColor }}>
      {showNumbers && (
        <div className="text-xs opacity-60 font-mono mb-2">
          Q{index + 1}
          {q.required && <span style={{ color: style.primaryColor }}> *</span>}
        </div>
      )}
                ...
                >
                  <span
                    ...
                    style={{
                      ...
                      background: sel ? style.primaryColor : 'transparent',
                    }}
                  >
              ...
              {Array.from({ length: q.scale ?? 5 }).map((_, i) => {
                ...
                return (
                  <button
                    key={i}
                    onClick={() => onChange(n)}
                    className="w-10 h-10 rounded-lg border-2 grid place-items-center font-medium transition"
                    style={{
                      borderColor: style.primaryColor,
                      background: sel ? style.primaryColor : 'transparent',
                      color: sel ? '#fff' : style.textColor,
                    }}
                  >
                    {n}
                  </button>
