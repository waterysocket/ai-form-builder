...
function DashboardPage() {
  ...
  const [deleteWarning, setDeleteWarning] = useState<{ id: string; isPublished: boolean } | null>(
    null,
  )
...
function SurveyCard({ survey, onDelete, onCopyLink }: any) {
  ...
  const typeLabel =
    surveyType === 'form' ? '📋 Form' : surveyType === 'quiz' ? '🧠 Quiz' : '📊 Survey'

  return (
    <div className="group rounded-2xl bg-surface-raised border border-border-subtle card-hover overflow-hidden flex flex-col">
      <Link to="/builder/$surveyId" params={{ surveyId: survey.id }} className="block flex-1">
        ...
        >
          ...
          >
            <div
              ...
              style={{ backgroundColor: `${style.primaryColor}40` }}
            />
            <div
            ...
            />
            <div
              className="h-2 rounded w-full mt-1"
        ...
        <div className="p-4">
          ...
          <div className="mt-3 flex items-center justify-between">
            ...
            >
              <span
                className={`w-1.5 h-1.5 rounded-full ${isPublished ? 'bg-brand' : 'bg-text-muted'}`}
              />
