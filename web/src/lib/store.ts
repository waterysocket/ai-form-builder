...
export const useSurveyStore = create<SurveyStoreState>((set, get) => ({
  ...
  addQuestion: (id, type) =>
    set((s) => ({
      surveys: s.surveys.map((x) =>
        x.id === id
          ? { ...x, questions: [...x.questions, { id: `q-${Date.now()}`, ...defaultQuestion(type) }] }
          : x,
      ),
    })),
  updateQuestion: (id, qid, patch) =>
    set((s) => ({
      surveys: s.surveys.map((x) =>
        x.id === id ? { ...x, questions: x.questions.map((q) => (q.id === qid ? { ...q, ...patch } : q)) } : x,
      ),
    })),
...
export const useAuth = create<AuthState>((set) => ({
  user: typeof window !== "undefined" && localStorage.getItem("fc-user")
